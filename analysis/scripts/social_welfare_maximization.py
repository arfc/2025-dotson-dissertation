...
if __name__ == "__main__":
    ...
    fig, axes = plt.subplots(1,2,figsize=(14,6), facecolor='w', sharey=True, sharex=True)
    ...
    axes[0].fill_between(x=x, y1=y1, y2=0.5, where=x<0.5, alpha=0.2, color='tab:orange')
    ...
    axes[1].fill_between(x=x, y1=y1, y2=p, where=x<p, alpha=0.2, color='tab:orange')
    axes[1].fill_between(x=x, y1=y1, y2=y2, where=((x>p) & (x<0.5)), alpha=0.2, color='gray')
    ...
    md = -1000
    ms = 0.0
    d = lambda x: md*(x - a1) + b1
    s = lambda x: ms*(x - a1) + b1
    y3 = d(x)
    y4 = s(x)
    ...
    axes[1].fill_between(x=x, y1=0, y2=p, where=x<q, alpha=0.2, color='tab:orange')
